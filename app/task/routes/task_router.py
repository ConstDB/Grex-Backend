...
async def get_task(workspace_id: int, task_id: int, token: dict = Depends(get_current_user), 
                   conn: asyncpg.Connection = Depends(get_db_connection)):
...
async def get_all_tasks(workspace_id: int,token: dict = Depends(get_current_user),
                         conn: asyncpg.Connection = Depends(get_db_connection)):
...
async def update_task(workspace_id: int, task_id: int, task_in: taskUpdate, 
                      token: dict = Depends(get_current_user),
                       conn: asyncpg.Connection = Depends(get_db_connection)):
...
async def delete_task(workspace_id: int, task_id: int, token: dict = Depends(get_current_user),
                      conn: asyncpg.Connection = Depends(get_db_connection)):
